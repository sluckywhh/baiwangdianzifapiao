...
+ (AFHTTPSessionManager *) geneSessionManager:(ClientConfig*)config
{
    ...
    if (client)
    {
        ...
        if ( eSerializationDataType == config.reqSerialType) {
            client.requestSerializer = [TSPutImageRequestSerializer serializer];
        } else if (eSerializationJsonType == config.reqSerialType) {
        ...
        if (eSerializationJsonType == config.respSerialType) {
            client.responseSerializer = [TSJSONResponseSerializer serializer];
        }
        else if (eSerializationTextType == config.respSerialType){
            [client.requestSerializer setValue:@"text/html" forHTTPHeaderField:@"Accept"];
            client.responseSerializer = [TSCompoundResponseSerializer serializer];
        }
        else {
            //notice: here the backend support json or xml.
